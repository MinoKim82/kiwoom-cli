...
* **다중 계좌 조회 시 (별칭 미지정 시 전체 계좌 합산 및 통합)**:
```text
==================================================
  [전체 계좌] 통합 평가 현황
==================================================
...
------------------------------------------------------------------------------------------
005930   | 삼성전자           | 100 주    | 70,000 원 | 75,000 원 |   +500,000 |  +7.14%
000660   | SK하이닉스          | 50 주     | 100,000 원| 110,000 원|   +500,000 | +10.00%
==========================================================================================
```

##### B. JSON 형식 (`--format json`)
* **다중 계좌 조회 시 (기본 동작)**:
  전체 개별 계좌별 데이터를 담은 `accounts` 리스트와, 이들의 모든 수치를 합산하고 종목을 결합한 `total` 객체를 함께 포함합니다.
  ```json
  {
    "accounts": [
      {
...
| :--- | :--- | :--- | :--- |
| `accounts` | Array | 없음 | 다중 계좌 순회 조회 시, 개별 계좌 단위의 상세 잔고 정보들을 담은 리스트 배열입니다. (단일 조회 시에는 미포함) |
| `total` | Object | 없음 | 다중 계좌 순회 조회 시, 전체 계좌의 자산 수치를 가중평균 및 합산하고 보유 주식 종목을 단일 결합한 요약 객체입니다. (단일 조회 시에는 미포함) |
