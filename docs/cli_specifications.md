...
> 
> **예시**:
> - `kiwoom -f json info` (전역 옵션으로 처리)
> - `kiwoom info -f json` (하위 명령어 뒤에서 덮어쓰기로 처리)
> 
...
### 2.2. `info` 명령어
키움증권 API 서버와 통신하여, 매핑된 계좌 별칭(Alias)의 **계좌 소유주 명칭(성명)**과 **상품 구분(예: 위탁종합 등)**의 상세 메타데이터를 조회합니다.

#### 입력 케이스 (Input Cases)
1. **모든 등록 계좌의 상세 정보 순회 조회 (텍스트 포맷)**
   ```bash
   kiwoom info
   ```
2. **모든 등록 계좌의 상세 정보 순회 조회 (JSON 포맷)**
   ```bash
   kiwoom info -f json
   ```
3. **특정 계좌에 대한 상세 정보 단일 조회 (텍스트 포맷)**
   ```bash
   kiwoom -a mh_default info
   ```
4. **특정 계좌에 대한 상세 정보 단일 조회 (JSON 포맷)**
   ```bash
   kiwoom -a mh_default info -f json
   ```
...
#### JSON 출력 필드 설명 및 의미 (JSON Schema Meanings)
| 필드명 | 데이터 타입 | 예시값 | 비즈니스적 의미 및 규격 |
| :--- | :--- | :--- | :--- |
| `account_alias` | String | `"mh_default"` | 조회 대상 계좌의 로컬 별칭(Alias)입니다. |
| `account_info` | Object | `{ ... }` | 키움 OpenAPI `/api/dostk/acnt` (api-id: `ka00001`)로부터 반환된 실시간 계좌 고유 정보 및 상태 메타데이터의 모음입니다. |
| `account_info.acctNo` | String | `"12345678"` | 키움증권 서버에서 리턴한 10자리 계좌번호 중 앞 **8자리**만 슬라이싱하여 노출하는 계좌번호입니다. |
| `account_info.acctNm` | String | `"홍길동"` | 해당 계좌를 소유하고 개설한 주주의 실제 법적 실명 또는 명칭입니다. |
| `account_info.acctTpNm` | String | `"위탁종합"` | 키움증권에서 정의한 해당 계좌의 상품 구분 정보입니다. (예: `위탁종합`, `해외주식`, `선물옵션` 등) |
| `account_info.return_code` | Integer | `0` | 키움 API 호출 처리 결과 코드입니다. `0`은 정상 호출 성공을 나타내며, 그 이외의 값이 설정될 경우 API 수준의 오류나 경고를 의미합니다. |
| `account_info.error` | String | `"..."` | 개별 계좌 상세 조회 실패 시 반환되는 예외 원인 메시지입니다. (오류 발생 시에만 제한적으로 포함) |
...
4. **특정 계좌의 잔고 현황 단일 조회 (JSON 포맷)**
   ```bash
   kiwoom -a mh_default balances -f json
   ```
5. **특정 실제 계좌번호를 지정하여 필터링 조회**
   ```bash
   kiwoom -a mh_default balances --acct 9876543204
   ```
...
* **입력 명령어 (Text 포맷)**:
  ```bash
  kiwoom -a invalid_alias balances
  ```
...
* **입력 명령어 (JSON 포맷)**:
  ```bash
  kiwoom -a invalid_alias balances -f json
  ```
...
특정 별칭을 한정하지 않고 **전체 계좌 목록을 조회(`kiwoom balances`)**하는 중에, 일부 계좌만 토큰 오류나 500 에러 등의 장애가 발생한 케이스입니다.
